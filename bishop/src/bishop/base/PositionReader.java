...
public class PositionReader extends PositionIo {
	...
	private long readPiece(final int color, final int recordedPieceIndex, final long occupiedSquares)
			throws IOException {
		...
		for (int square = Square.FIRST; square < Square.LAST && remainingPieceCount > 0; square++) {
			...
			if ((possibleMask & squreMask) != 0) {
				final IProbabilityModel model = getPiecePositionProbabilityModel(remainingPieceCount,
