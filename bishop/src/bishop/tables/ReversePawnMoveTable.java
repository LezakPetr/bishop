...
public class ReversePawnMoveTable {
	...
	private static long[] createTable() {
		...
		for (int color = Color.FIRST; color < Color.LAST; color++) {
			for (int square = Square.FIRST; square < Square.LAST; square++) {
		    	...
		    	if ((color == Color.WHITE && rank >= Rank.R3) || (color == Color.BLACK && rank <= Rank.R6)) {
			        ...
			        if ((color == Color.WHITE && rank == Rank.R4) || (color == Color.BLACK && rank == Rank.R5))
